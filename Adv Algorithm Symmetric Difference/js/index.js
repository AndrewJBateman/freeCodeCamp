...
function uniqueArray(arr1, arr2){
  var newArray = arr1.concat(arr2); //combine arrays in new array with concat method
  ...
  }); //end newArray Filter function
} //end uniqueArray function

//Helper Function2 to remove duplicates in each sub-array
function extractDuplicates(arr){
  var endResult = []; //empty array variable
...
/*Main function. Convert argument objects to a real array using [].slice function
...
function sym(args) {
  ...
  return args.reduce(function(acc, next) {
    ...
    return acc;
  }, []); //end args reduce function. Note no default value is needed - [] at end of function are not required.
} //end sym function

var result = sym([1, 2, 3], [5, 2, 1, 4]);
console.log(result);